...
const DayRow = (props: DayRow) => {
  ...
  function isDateInThisWeek(date:any) {
  ...
  for (const key in myDataAccordintToMe) {
    if (key == dayDate && isDateInThisWeek(new Date(dayDate))) {
      Status = true;
              ...
              day.openIntervals.map((res: any, index: number) => {
                return (
                  <>
                    <div className="time-group" key={index}>
                      <span className="time-b">
                        {OpenStausFunctions.formatTime(res.start).replace(':00','').replace(" ", "")}
                      </span>{" "}
                      <span className="dash"></span>{" "}
                      <span className="time-b">
                        {OpenStausFunctions.formatTime(res.end).replace(':00','').replace(" ", "")}
                      </span>
                    </div>
                  </>
                );
                  ...
                  return openint.openIntervals.map((res: any,index:number) => {
                    return (
                      <>
                        <div className="time-group" key={index}>
                          <span className="time-b">
                            {OpenStausFunctions.formatTime(res.start).replace(':00','')}
                          </span>{" "}
                          <span className="dash"></span>{" "}
                          <span className="time-b">
                            {OpenStausFunctions.formatTime(res.end).replace(':00','')}
                          </span>
                        </div>
                      </>
                    );
