...
import { conversionDetailsPhone } from "../../sites-global/global";
...
const LocationCard: CardComponent<Location> = (props: any) => {
  ...
  return props.result.map((result: any, keyindex: number) => {
    ...
    //   url = props.mode === "development" ? `/location/${result.rawData.id}?locale=en_GB` : `/${result.rawData.id}-${result1}`;
    // } else {
    //   url = props.mode === "development" ? `/location/${result.rawData.slug}?locale=en_GB` : `/${finalslug}`;
    // }
    return (
      {name}
